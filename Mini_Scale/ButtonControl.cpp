  ...
  switch (btnState) {
    ...
    case BTN_DEBOUNCE_RELEASE:
      ...
      // Кнопка отпущена — определяем действие по длительности
      {
        ...
        // Перезапускаем таймер окна — полные MENU_CONFIRM_WINDOW_MS с момента отпускания,
        // а не с момента достижения порога удержания (иначе реальное окно слишком короткое).
        if (menuPromptActive) {
          menuPromptTime = now;
        }
        return BTN_NONE;
